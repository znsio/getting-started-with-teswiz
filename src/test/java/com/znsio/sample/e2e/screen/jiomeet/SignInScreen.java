...
import com.znsio.sample.e2e.screen.android.jiomeet.SignInScreenAndroid;
import com.znsio.sample.e2e.screen.web.jiomeet.SignInScreenWeb;
...
public abstract class SignInScreen {
    ...
    public static SignInScreen get() {
        Driver driver = Drivers.getDriverForCurrentUser(Thread.currentThread()
                                                              .getId());
        Platform platform = Runner.fetchPlatform(Thread.currentThread()
                                                       .getId());
        LOGGER.info(SCREEN_NAME + ": Driver type: " + driver.getType() + ": Platform: " + platform);
        Visual visually = Drivers.getVisualDriverForCurrentUser(Thread.currentThread()
                                          .getId());
