...
import com.znsio.sample.e2e.businessLayer.amzon.AmazonItemsViewBL;
...
public class AmazonSteps {
    ...
    public void iAGuestUserSearchForItemInSearchBar(String item) {
        ...
        new AmazonHomeBL(SAMPLE_TEST_CONTEXT.GUEST_USER, Runner.platform).doItemSearch(item);
    }

    @And("I select the {string} item from the search results")
    public void iSelectTheItemFromTheSearchResults(String position) {
        new AmazonItemsViewBL().viewItemByPosition(position);
    }
    ...
    public void itemShouldBeAddedToTheCart() {
        new AmazonCartsBL().isItemAddedToCart();
    }
}
