...
Feature: Search products on amazon
  ...
  #  CONFIG=./configs/amazon_search_local_config.properties TAG="@amazonSearch" PLATFORM=android ./gradlew run
  @web @android
  Scenario: As a guest user i should be able to search for product on amazon search and add it to cart
    Given I, a guest user search for an "Iphone 13" product
